...
/*   Updated: 2023/11/29 04:46:06 by ajawad           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_return(char *buffer, char *read_buff, char **array)
{
	if (ft_strlen(buffer))
		...
		return (free(*array), *array = NULL, free(read_buff), \
				free(buffer), NULL);
	else
...
char	*ft_get_line(int fd, char *buffer, char **array, char *read_buff)
{
	...
	{
		buffer = ft_strjoin(buffer, *array);
	...
	while (1)
	{
		...
		if (ft_check_newline(read_buff) == 1)
			return (*array = ft_get_remaining(read_buff, 0), \
					free(read_buff), buffer);
	}
	return (NULL);
}
...
char	*get_next_line(int fd)
{
	...
	if (!read_buff)
		return (NULL);
	buffer = ft_get_line(fd, buffer, &array[fd], read_buff);
	return (buffer);
}
